(window["webpackJsonp"] = window["webpackJsonp"] || []).push([["cards~landing~search-room"],{
...
/* WEBPACK VAR INJECTION */(function($) {/* harmony import */ var _datepicker__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! ./datepicker */ "./blocks/calendar/datepicker.js");
...
$(function () {
  ...
  $('.start,.end').on('click', function (event) {
    var calendarContainer = event.target.closest('.calendar-container');
    var start = calendarContainer.querySelector('.start');
    var end = calendarContainer.querySelector('.end');
    var picker = $(start).datepicker({
      ...
      onShow: function onShow(dp, complete) {
        if (complete) isPickerVisible = true;
      },
      onHide: function onHide(dp, complete) {
        if (complete) isPickerVisible = false;
      }
    }).data('datepicker');
    $('.start, .end').on('click', function () {
      if (isPickerVisible) picker.hide();else // invisibile
        picker.show();
    });
  });
});
...
var dropdownOpen = function dropdownOpen(event) {
  ...
  var dropdownItem = target.closest('.dropdown__item');
  var inputFieldsArr = document.querySelectorAll('.input__field');
  var dropdownExpandsArr = document.querySelectorAll('.dropdown__expand');

  if (dropdownItem) {
    var inputField = dropdownItem.querySelector('.input__field');
    var dropdownExpand = dropdownItem.querySelector('.dropdown__expand');
    var submitBtn = dropdownItem.querySelector('button[value = "submit"]');
  ...
  if (!dropdownItem) {
    inputFieldsArr.forEach(function (item) {
      item.classList.remove('input__field--expanded');
    });
    dropdownExpandsArr.forEach(function (item) {
...
var changeover = function changeover(event) {
  ...
  if (dropdownItem) {
    var inputField = dropdownItem.querySelector('.input__field');
    var counterValueArr = dropdownItem.querySelectorAll('.counter__value');
    var counterButtons = target.closest('.counter__buttons');
    var amountBabies = dropdownItem.querySelector('input[name = "babies"]');
    var amountChildren = dropdownItem.querySelector('input[name = "children"]');
    var amountAdults = dropdownItem.querySelector('input[name = "adults"]');
    var amountBedrooms = dropdownItem.querySelector('input[name = "bedrooms"]');
    var amountBeds = dropdownItem.querySelector('input[name = "beds"]');
    var amountBathrooms = dropdownItem.querySelector('input[name = "bathrooms"]');
    var resetBtn = dropdownItem.querySelector('button[value = "reset"]');
    var totalGuests = 0;

    if (counterButtons) {
      var counterValue = counterButtons.querySelector('.counter__value');
      var counterBtnMinus = counterButtons.querySelector('.counter__btn--minus');
      var counterBtnPlus = counterButtons.querySelector('.counter__btn--plus');

      if (target === counterBtnMinus && counterValue.value !== '0') {
        var counter = Number(counterValue.value);
        counter -= 1;
      ...
      if (target === counterBtnPlus) {
        var _counter = Number(counterValue.value);
    ...
    var reset = function reset() {
      var counterButtonsArr = dropdownItem.querySelectorAll('.counter__buttons');
      counterButtonsArr.forEach(function (item) {
        var counter = item.querySelector('.counter__value');
    ...
    var numericCases = function numericCases(name) {
      if (name === 'guests') {
        var babies = Number(amountBabies.value);
        var children = Number(amountChildren.value);
        var adults = Number(amountAdults.value);
        var otherGuests = children + adults;
        var caption = '';
        var babiesCaption = '';
        var isBabies = '';
        ...
        isBabies = babies ? ", ".concat(babies, " ").concat(babiesCaption) : '';
        inputField.value = otherGuests ? "".concat(otherGuests, " ").concat(caption).concat(isBabies) : 'Сколько гостей';
      }

      if (name === 'rooms') {
        var bedrooms = Number(amountBedrooms.value);
        var beds = Number(amountBeds.value);
        var bathrooms = Number(amountBathrooms.value);
        var bedroomsCaption = '';
        var bedsCaption = '';
        var bathroomsCaption = '';
        ...
        var inputBedrooms = bedrooms ? "".concat(bedrooms, " ").concat(bedroomsCaption) : '';
        var inputBeds = beds ? "".concat(beds, " ").concat(bedsCaption) : '';
        var inputBathrooms = bathrooms ? "".concat(bathrooms, " ").concat(bathroomsCaption) : '';
        var inputValue = 'Выберите удобства';
        var valuesArray = [inputBedrooms, inputBeds, inputBathrooms].filter(function (item) {
        ...
        inputField.value = inputValue.length > 20 ? "".concat(inputValue.slice(0, 20), "...") : inputValue;
      }
    };
...
//# sourceMappingURL=cards~landing~search-room.js.map